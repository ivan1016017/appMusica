...
export class AlbumListComponent implements OnInit {
    ...
    .subscribe(albumes => {
      ...
      if(albumes.length>0){
        this.mostrarAlbumes.map((currentElement, index) => {
          this.albumService.getCancionesAlbum(currentElement.id,this.token).subscribe(canciones => {
  ...
  onSelect(a: Album, index: number){
    ...
    .subscribe(canciones => {
      ...
      // this.albumSeleccionado.usuarios_en_album = ["flag"]
      this.usuarioService.getUsuario(this.userId).subscribe(usuario => {
      })
