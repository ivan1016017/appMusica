...
export class CancionListComponent implements OnInit {
  ...
  onSelect(cancion: Cancion, indice: number){
    ...
    this.cancionService.getAlbumesCancion(this.token, cancion.id)
    .subscribe(albumes => {
  ...
  buscarCancion(busqueda: string, filter = this.selectedFilter){
    ...
    function ordenarAlfabet (a: Cancion, b:Cancion) {
      if (a.titulo > b.titulo) {
        return 1;
      }
      if (a.titulo < b.titulo) {
    ...
    this.listaCanciones = cancionesBusqueda.sort(ordenarAlfabet)
    function compararFavorita (a: Cancion, b:Cancion){return Number(b.es_favorita) - Number(a.es_favorita);}
    this.mostrarCanciones = this.listaCanciones.sort(compararFavorita)
  }
  ...
  changeFavorite() {
    ...
    this.cancionService.changeFavoriteState(this.token, this.cancionSeleccionada.id)
      .subscribe(cancion => {
  ...
  eliminarCancion(){
    this.cancionService.eliminarCancion(this.token, this.cancionSeleccionada.id)
    .subscribe(cancion => {
      this.ngOnInit()
      this.showSuccess()
    },
